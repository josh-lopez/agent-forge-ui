 ...
 *
 * Covers:
 *   AC1  – additional mixed-fixture fractions (2/3, 3/4, etc.)
 ...
 *   AC4  – zero-deliveries: formatSuccessRate(null) returns '—' (not '0.0%')
...
describe('calculateSuccessRate — supplemental', () => {
  // AC1: 2 of 3 delivered → ~0.6667
  it('calculates 2/3 correctly (~0.6667)', () => {
  ...
  // AC1: 3 of 4 delivered → 0.75
  it('calculates 3/4 correctly (0.75)', () => {
    const events: DeliveryEvent[] = [
      makeEvent('1', 'delivered'),
      makeEvent('2', 'delivered'),
    ...
    expect(calculateSuccessRate(events)).toBe(0.75);
  });
  ...
  it('treats exhausted status as non-delivered', () => {
    ...
    // 1 delivered out of 4 → 0.25
    expect(calculateSuccessRate(events)).toBe(0.25);
  });
  ...
  it('returns 1 for a large all-delivered dataset', () => {
  ...
  it('aggregates correctly across many distinct event types', () => {
    ...
    // 1 delivered per type (5 total), 1 failed per type (5 total) → 0.5
...
describe('formatSuccessRate — supplemental', () => {
  // AC4: null → '—' (not '0.0%' or 'NaN%')
  it('returns "—" for null, not "0.0%" or "NaN%"', () => {
...
describe('mountMetricsDashboard — supplemental', () => {
  ...
  it('handles multiple sequential update() calls correctly', () => {
    ...
    // First update: 1/2 delivered → 50.0%
    ...
    // Second update: 0/2 delivered → 0.0%
    ...
    // Third update: 2/2 delivered → 100.0%
  ...
  it('displays correct rate when some events are exhausted (not delivered)', () => {
    ...
    expect(valueEl!.textContent!.trim()).toBe('25.0%');
  });
