...
# Count H1 headings, ignoring lines inside fenced code blocks (e.g. shell
# comments such as "# Build ..." inside ```bash blocks are NOT Markdown H1s).
H1_COUNT=$(awk '/^```/{infence=!infence; next} !infence && /^# /{c++} END{print c+0}' "$README")
